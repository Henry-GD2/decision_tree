...
def main():
    df = pd.read_csv('../DataSet/西瓜数据集2.0.csv')
    data = df.values[:10, 1:].tolist()
    test = df.values[10:, 1:].tolist()
    labels = df.columns.values[1:-1].tolist()
    labels_full = labels[:]
    myTree = CART_creatTree(data, labels, test)#基于基尼系数最小
    pT.createPlot(myTree)
    myTree = post_pruning(myTree, data, test, labels_full)#基于损失函数最小化的剪枝
    print('myTree', myTree)
    pT.createPlot(myTree)
