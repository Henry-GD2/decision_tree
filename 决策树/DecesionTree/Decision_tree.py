...
def classify(input_Tree, feat_labels, test_Vec):
    ...
    class_label = ''
    if '<=' in first_str:
        feat_value = float(re.compile("(<=.+)").search(first_str).group()[2:])
        feat_key = re.compile("(.+<=)").search(first_str).group()[:-2]
        second_dict = input_Tree[first_str]
        feat_index = feat_labels.index(feat_key)
        if test_Vec[feat_index] <= feat_value:
            judge = 1
        else:
            judge = 0
    ...
    for key in second_dict.keys():
        if type(second_dict[key]).__name__ == 'dict':
            if type(dataset[0][label_index]).__name__ == 'str':
                ...
                input_tree[first_str][key] = post_pruning(second_dict[key],
                                                          splitContinuousDataSet(dataset, label_index, feat_value, key),
                                                          splitContinuousDataSet(data_test, label_index, feat_value,
                                                                                 key), temp_labels)
...
def main():
    df = pd.read_csv('../DataSet/adult-stretch.csv')
    data = df.values[:16, 1:].tolist()
    data_full = data[:]
    test = df.values[16:, 1:].tolist()
    labels = df.columns.values[0:-1].tolist()
    labels_full = labels[:]
    myTree = ID3_createTree(data, labels, data_full, labels_full, test)
    pT.createPlot(myTree)
    # y_test = classifyTest(myTree, labels_full, test)
    # y_true = df.values[120:, -1].tolist()
    # print(cal_acc(y_test, y_true))
    # print('myTree', myTree)
    # myTree = post_pruning(myTree, data_full, test, labels_full)
    # print('myTree', myTree)
    # pT.createPlot(myTree)
    Tree1 = {'花瓣长度<=2.45': {0: {'花瓣宽度<=1.75': {0: {'花萼宽度<=3.1': {0: 'versicolor', 1: 'virginica'}}, 1: {'花瓣长度<=4.95': {0: 'virginica', 1: 'versicolor'}}}}, 1: 'setosa'}}
    Tree2 = {'花瓣长度<=2.45': {0: {'花瓣宽度<=1.75': {0: 'virginica',
                                               1: {'花瓣长度<=4.95': {0: 'virginica', 1: 'versicolor'}}}}, 1: 'setosa'}}
    Tree3 = {'act': {'STRETCH': 'T', 'DIP': {'age': {'ADULT': 'T', 'CHILD': 'F'}}}}
    Tree4 = {'act': {'STRETCH': 'T', 'DIP': 'F'}}
    # Tree5 = {'Color intensity<=3.82': {0: {'Flavanoids<=1.475': {0: {'Proline<=670.0': {0: 'wine_1', 1: {'Ash<=3.19': {0: 'wine_1', 1: 'wine_2'}}}}, 1: 'wine_3'}}, 1: {'Proline<=978.5': {0: 'wine_1', 1: 'wine_2'}}} }
    # Tree5 = {'Color intensity<=3.82': {0: 'wine_3',
    #                         1: {'Proline<=978.5': {0: 'wine_1', 1: 'wine_2'}}}}

    Tree5 = {'Color intensity<=3.82': {0: {
        'Flavanoids<=1.475': {0: {'Proline<=670.0': {0: 'wine_1', 1: 'wine_2'}},
                              1: 'wine_3'}}, 1: {'Proline<=978.5': {0: 'wine_1', 1: 'wine_2'}}}}
    y_test = classifyTest(Tree4, labels_full, test)
    y_true = df.values[16:, -1].tolist()
    print(cal_acc(y_test, y_true))
    pT.createPlot(Tree5)
